jQuery(document).ready(function(){
	...
	welcome_panel_slider_enable();

	jQuery(".kkb-arrow-left").find(".btn-product-left").click(function(){
		var all_count = jQuery(this).attr("data-all");
		var current   = jQuery(this).attr("current-num");
		if(current > 0){
			if(current == 1){
				jQuery(".kkmarket-wrapper").find(".kkb-product-wrapper").animate({'left' : '0px'}, 300);
			} else {
				jQuery(".kkmarket-wrapper").find(".kkb-product-wrapper").animate({'left' : ( '-'+((current*1)-1) * 320)+'px'}, 300);
			}
			jQuery(this).attr("current-num", (current*1)-1);
			jQuery(".kkb-arrow-right").find(".btn-product-right").attr("current-num", (current*1));
		}
	});

	jQuery(".kkb-arrow-right").find(".btn-product-right").click(function(){
		var all_count = jQuery(this).attr("data-all");
		var current   = jQuery(this).attr("current-num");
		if( (current + 1) <= all_count){
			jQuery(".kkmarket-wrapper").find(".kkb-product-wrapper").animate({'left' : ( '-'+(current*1) * 320)+'px'}, 300);
			jQuery(this).attr("current-num", (current*1)+1);
			jQuery(".kkb-arrow-left").find(".btn-product-left").attr("current-num", (current*1));
		} else {
			jQuery(".kkmarket-wrapper").find(".kkb-product-wrapper").animate({'left' : '0px'}, 300);
			jQuery(this).attr("current-num", 1);
			jQuery(".kkb-arrow-left").find(".btn-product-left").attr("current-num", 1);
		}
	});
...
function welcome_panel_slider_enable(){
	jQuery(".kkb-arrow-right").find(".btn-product-right").trigger('click');
	setTimeout(welcome_panel_slider_enable, 5000);
}
